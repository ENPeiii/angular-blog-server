...
以 `Post` 有多個 `Comment` 為例（一篇文章可以有很多留言，一個留言只屬於一篇文章）。
...
model Post {
  // ...其他欄位
  comments Comment[]   // ← 一篇 Post 有多個 Comment（反向關聯）
}

model Comment {
  // ...其他欄位
  postId String        // ← 外鍵欄位（存 Post.id）
  post   Post   @relation(fields: [postId], references: [id], onDelete: Cascade)
}
```

`onDelete: Cascade`：刪除 Post 時，旗下所有 Comment 也一起刪除。

### Step 2 — 執行 Migration

```bash
npx prisma migrate dev --name "post_comment_relation"
```
...
// models/post.ts
export interface Post {
  comments: Comment[];  // 或只用 PublicComment[]
}
